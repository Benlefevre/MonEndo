...
class SymptomDaoTest {
    ...
    @Test
    fun insertAndGetAllSymptoms_success_correctDataReturned() {
        val symptom = Symptom(rowId,"burns",date)
        val symptom2 = Symptom(rowId2,"fever",date)
    ...
    @Test
    fun deleteAllSymptoms_success_noDataReturned() {
    ...
    @Test
    fun getPainSymptom_success_correctDataReturned() {
        val symptom = Symptom(rowId,"burns",date)
        val symptom2 = Symptom(rowId2,"fever",date)
        val symptomList = listOf(symptom,symptom2)
    ...
    @Test
    fun getSymptomByPeriod_success_correctDataReturned() {
        ...
        val symptom = Symptom(rowId,"bloating",date1)
        val symptom2 = Symptom(rowId2,"cramps",date2)
        ...
        symptomReturned = endoDatabase.symptomDao().getSymptomsByPeriod(date3,date).getOrAwaitValue()
        assertEquals(0,symptomReturned.size)

        symptomReturned = endoDatabase.symptomDao().getSymptomsByPeriod(date4,date1).getOrAwaitValue()
        assertEquals(1,symptomReturned.size)
        assertEquals(symptom,symptomReturned[0])

        symptomReturned = endoDatabase.symptomDao().getSymptomsByPeriod(date1,date3).getOrAwaitValue()
        assertEquals(2,symptomReturned.size)
        assertEquals(symptom,symptomReturned[0])
        assertEquals(symptom2,symptomReturned[1])
